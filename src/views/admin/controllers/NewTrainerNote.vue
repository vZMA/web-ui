...
export default {
	name: 'EditTrainerNote',
	title: 'Edit Trainer Note',
	data() {
		return {
			description: '',
			content: '',
		...
		async submitForm() {
			...
			if (this.content && this.description)
				...
				try {	// Hit the local database to Finalize the record
					const {data} = await zabApi.post(`controller/notes/createtnote`, {
						cid: this.cid,
					...
					// Put a little message on screen saying success
						this.toastSuccess('Trainer Note Submitted');					
						window.history.back();
